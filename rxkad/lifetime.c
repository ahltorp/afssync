...
RCSID("$Id: lifetime.c,v 1.1 2005/01/17 10:02:38 lha Exp $");

/* If you want to disable this feature */
int krb4_no_long_lifetimes = 0;
...
krb4_life_to_time(uint32_t start, int life_)
{
    ...
    if (krb4_no_long_lifetimes) return start + life*5*60;
...
int krb_time_to_life(uint32_t start, uint32_t end)
{
    ...
    if (krb4_no_long_lifetimes) return (lifetime + 5*60 - 1)/(5*60);
...
krb4_life_to_atime(int life)
{
    ...
    if (life == TKTLIFENOEXPIRE && !krb4_no_long_lifetimes)
	return("Forever");
    when = krb4_life_to_time(0, life);
    secs = when%60;
    when /= 60;
    mins = when%60;
    when /= 60;
