...
RCSID("$Id: plwp.c,v 1.2 2008/03/22 17:03:24 ahltorp Exp $");
...
static void Free_PCB(PROCESS pid);
static void Cal_Highest_runnable_priority(void);
...
/* XXX pthread_exit() may be dynamically loaded, no good in chroot */
#define LWP_INT_EXIT()		pthread_exit(NULL)
...
#define LWP_INT_EXIT()		ExitThread(0); /* XXX */
...
/*
 * End thread `pid', internal cleanup.
 *
 * If `pid' is currently executing, we assume that it is finished and
 * about to return from the thread routine.
 */
int 
LWP_DestroyProcess(PROCESS pid)
{
	...
	if (lwp_cpptr == pid){
	    ...
	    LWP_INT_SIGNAL(&lwp_cpptr->c);
	    LWP_INT_UNLOCK(&ct_mutex);
	    /* LWP_INT_EXIT(); no need, implicit call when thread returns */
	} else {
...
static void 
Cal_Highest_runnable_priority(void)
{
    int	i;
