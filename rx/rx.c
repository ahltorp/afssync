...
RCSID("$Id: rx.c,v 1.1.1.1 2008/03/22 16:58:37 ahltorp Exp $");
...
rx_Init(uint16_t port)
{
    ...
    rx_socket_icmp = rxi_GetICMPSocket();
...
{
    osi_socket asocket = OSI_NULLSOCKET;
	...
	if (service) {
		 ...
		 */
		asocket = service->socket;
		reuse = 1;
	    }
	} else {
	    if (asocket == OSI_NULLSOCKET) {

		/*
		 * If we don't already have a socket (from another service on
		 * same port) get a new one
		 */
		asocket = rxi_GetUDPSocket(port, NULL);
		if (asocket == OSI_NULLSOCKET) {
		    USERPRI;
		    rxi_FreeService(tservice);
		    return 0;
...
rxi_FindService(osi_socket asocket, uint16_t serviceId)
{
    struct rx_service **sp;
    DPRINT(("rxi_FindService service: %d\n", serviceId));

    for (sp = &rx_services[0]; *sp; sp++) {
        DPRINT(("rxi_FindService search service: %d\n", (*sp)->serviceId));
	if ((*sp)->serviceId == serviceId && (*sp)->socket == asocket)
	    return *sp;
    }
    return 0;
}
...
rxi_FindConnection(osi_socket asocket, uint32_t host,
		   ...
		   uint32_t epoch, int type, u_int securityIndex)
{
    ...
    DPRINT(("rxi_FindConnection 1 conn: %p\n", conn));
    if (conn != NULL) {
	if (pp->host != host || pp->port != port)
	    conn->peer = rxi_ChangePeer (pp, host, port);
    } else {
	struct rx_service *service;
        DPRINT(("rxi_FindConnection 2\n"));

	if (type == RX_CLIENT_CONNECTION)
	    return (struct rx_connection *) 0;
        DPRINT(("rxi_FindConnection 3\n"));
	service = rxi_FindService(asocket, serviceId);
        DPRINT(("rxi_FindConnection service: %p\n", service));
...
/*
 * Force a timeout on the connection for `host', `port'.
 */

void
rxi_KillConnection(uint32_t host, u_short port)
{
    struct rx_connection *conn;
    int hashindex, i;
    
    for (hashindex = 0; hashindex < rx_hashTableSize; hashindex++) {
	for (conn = rx_connHashTable[hashindex]; conn; conn = conn->next) {
	    if (htonl(conn->peer->host) == host && 
		htons(conn->peer->port) == port)
	    {
		for (i = 0; i < RX_MAXCALLS; i++) {
		    struct rx_call *call = conn->call[i];
		    if (call && call->state == RX_STATE_ACTIVE)
...
struct rx_packet *
rxi_ReceivePacket(struct rx_packet * np, osi_socket asocket,
 ...
 */
	...
	return rxi_ReceiveVersionPacket(np, asocket, host, port);

    if (np->header.type == RX_PACKET_TYPE_DEBUG)
	return rxi_ReceiveDebugPacket(np, asocket, host, port);
    ...
    DPRINT(("rxi_RecivePacket %s\n", type == RX_SERVER_CONNECTION ? "server*************************************************************":"client"));
    
    /*
     * Find the connection (or fabricate one, if we're the server & if
     * necessary) associated with this packet
     */
    conn = rxi_FindConnection(asocket, host, port, np->header.serviceId,
			      np->header.cid, np->header.epoch, type,
			      np->header.securityIndex);

    DPRINT(("rxi_RecivePacket conn: %p\n", conn));
    if (!conn) {
	/*
    ...
    DPRINT(("rxi_RecivePacket conn->error: %d\n", conn->error));
    /*
    ...
    if (type == RX_SERVER_CONNECTION) {/* We're the server */
        DPRINT(("******** rxi_RecivePacket server processing **********\n"));
...
rxi_ScheduleDecongestionEvent(struct rx_call *call, int nPackets)
{
    ...
    rxevent_Post(&tmp, rxi_DecongestionEvent, (void *) peer, (void *)(long)nPackets);
}
