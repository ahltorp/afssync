...
RCSID("$Id: rx_rdwr.c,v 1.1.1.1 2008/03/22 16:58:37 ahltorp Exp $");

ssize_t
rx_Read(struct rx_call *call, void *vbuf, size_t nbytes)
{
    struct rx_packet *rp;
    size_t requestCount;
    char *buf = (char *)vbuf;
...
ssize_t
rx_Write(struct rx_call *call, const void *vbuf, size_t nbytes)
{
    struct rx_connection *conn = call->conn;
    size_t requestCount = nbytes;
...
rx_FlushWrite(struct rx_call *call)
{
	...
	if (call->currentPacket) {
	    ...
	    cp->length = 0;
	    cp->niovecs = 2;	       /* just the header + sec header */
	    call->nFree = 0;
	}
