...
RCSID("$Id: rx_pkt.c,v 1.2 2008/10/26 20:28:48 ahltorp Exp $");
...
void
rxi_ReadIcmp(osi_socket asocket)
{
    ...
    length = recv(asocket, buffer, sizeof(buffer), 0);
    if (length < 0 || length > sizeof(buffer))
	return;

    p = buffer;

    if (length < (5 * 4) + 8 + (5 * 4) + 8)
	return;
    if (*p != 0x45) /* header comes with options, punt */
	return;
...
#endif
    if (p[5 * 4] != ICMP_UNREACH) 
	return;
    if (p[6 * 4] == ICMP_UNREACH_NEEDFRAG) /* ignore need frag for now */
	return;
    if (p[(5 * 4) + 8] != 0x45) /* header comes with options, punt */
	return;
 ...
 */
int 
rxi_ReadPacket(int asocket, struct rx_packet *p,
...
/* Send a udp datagram */
int 
osi_NetSend(osi_socket asocket, char *addr, struct iovec *dvec,
	    int nvecs, int length)
{
	...
	FD_SET(asocket, &sfds);
	while ((error = select(asocket + 1, 0, &sfds, 0, 0)) != 1) {
	    if (error >= 0 || errno != EINTR)
		osi_Panic("osi_NetSend: select error %d.%d", error, errno);
	}
    }
	       ...
	       struct rx_packet *p)
{
     ...
     * received on
     */
    asocket = (conn->type == RX_CLIENT_CONNECTION
	 ...
	 */
	GLOBAL_UNLOCK();
	if (osi_NetSend(asocket, (char *)&addr, p->wirevec, 
