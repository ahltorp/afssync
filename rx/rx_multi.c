...
RCSID("$Id: rx_multi.c,v 1.1.1.1 2008/03/22 16:58:37 ahltorp Exp $");

static void multi_Ready(struct rx_call *, struct multi_handle *, void *);
...
multi_Init(struct rx_connection **conns, int nConns)
{
	...
	rx_SetArrivalProc(call, multi_Ready, mh, NULL);
    }
    return mh;
}
 ...
 */
static void 
multi_Ready(struct rx_call *call, struct multi_handle *mh, void *unused)
{
    int callindex;
    for (callindex = 0; callindex < RX_MAXCALLS; callindex++)
	if (mh->calls[callindex] == call)
	    break;
    if (callindex >= RX_MAXCALLS)
	abort();
    *mh->firstNotReady++ = callindex;
    mh->nReady++;
    osi_rxWakeup(mh);
}
