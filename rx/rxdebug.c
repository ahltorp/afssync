...
RCSID ("$Id: rxdebug.c,v 1.1.1.1 2008/03/22 16:58:37 ahltorp Exp $");
	  ...
	  long alen, char *aresult, long aresultLen)
{
	...
	if (code > 0) {
	    ...
	    socklen_t faddrLen = sizeof(struct sockaddr_in);
...
static int nodally_flag;
static int allconns_flag;
static int rxstats_flag;
static int onlyServer_flag;
static int onlyClient_flag;
static int version_flag;
static int noConns_flag;
...
MainCommand (void)
{
	...
	if(!withRxStats) {
	    ...
	    if ((code == sizeof(tin)) &&
		(ntohl(((struct rx_debugIn *)(void*)&rxstats)->type) ==
		 RX_DEBUGI_BADTYPE)) {
		fprintf (stderr, "WARNING: Server doens't support "
			 "retrieval of Rx statistics\n");
	    } else {
    ...
    if (onlyServer_flag) printf ("Showing only server connections\n");
    if (onlyClient_flag) printf ("Showing only client connections\n");
    if (onlyAuth != 999) {
	static char *name[] =
	...
	if (onlyServer_flag && (tconn.type != RX_SERVER_CONNECTION)) continue;
	if (onlyClient_flag && (tconn.type != RX_CLIENT_CONNECTION)) continue;
	if (onlyAuth != 999) {
	    if (onlyAuth == -1) {
...
static struct agetargs args[] = {
    ...
    {"nodally", 0, aarg_flag, &nodally_flag,
     "don't show dallying conns", NULL },
    {"allconnections", 0, aarg_flag, &allconns_flag,
     "don't filter out uninteresting connections on server"},
    {"rxstats", 0, aarg_flag, &rxstats_flag,
     "show Rx statistics", NULL },
    {"onlyserver", 0, aarg_flag, &onlyServer_flag,
     "only show server conns", NULL },
    {"onlyclient", 0, aarg_flag, &onlyClient_flag,
    ...
    {"noconns", 0, aarg_flag, &noConns_flag,
     "show no connections", NULL },
    {"help", 0, aarg_flag, &help_flag,
     "show help", NULL },
    {NULL}
};
...
main(int argc, char **argv)
{
    ...
    if (agetarg (args, argc, argv, &o, AARG_AFSSTYLE)) {
        usage();
        return 0;
    }
