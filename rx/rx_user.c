...
RCSID("$Id: rx_user.c,v 1.1.1.1 2008/03/22 16:58:37 ahltorp Exp $");
...
/*
 * Add the socket to the listing queue and add it to the select mask.
 */

static int
addSocket(const char *name, osi_socket socketFd)
{
    osi_socket *sockets;

    if (socketFd >= FD_SETSIZE) {
	osi_Msg(("%s: socket fd too large\n", name));
    ...
    if (sockets == NULL) {
	perror("socket");
	osi_Msg(("%sunable to allocated memory for socket\n", name));
	return 1;
    }
    rx_sockets = sockets;
...
{
    int socketFd = -1;
    char *name = "rxi_GetICMPSocket: ";

    socketFd = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP);
    if (socketFd < 0) {
	if (getuid() == 0) {
	    perror("socket");
	    osi_Msg(("%sunable to create ICMP socket\n", name));
	}
	goto error;
    }
...
rxi_Listener(void)
{
	...
	lastPollWorked = 0;	       /* default is that it didn't find
        ...
        DPRINT(("rxi_Listener doing IOMGR_Select\n"));
	fds = IOMGR_Select (rx_maxSocketNumber + 1, &rfds, 0, 0, tvp);
        DPRINT(("rxi_Listener IOMGR_Select returning: %d\n", fds));
	clock_NewTime();
	if (fds > 0) {
	    if (doingPoll)
		lastPollWorked = 1;

	    for (i = 0; i < num_rx_sockets; i++) {
		if (p == NULL)
		    break;
		else if (FD_ISSET(rx_sockets[i], &rfds)) {
                    DPRINT(("rxi_Listener rx_socket: %d\n", i));
		    if (rx_sockets[i] == rx_socket_icmp) {
                        DPRINT(("rxi_Listener rx_socket icmp\n", i));
			rxi_ReadIcmp(rx_sockets[i]);
		    } else if (rxi_ReadPacket(rx_sockets[i],p, &host, &port)) {
                        DPRINT(("rxi_Listener rx_socket real data\n", i));
			p = rxi_ReceivePacket(p, rx_sockets[i], host, port);
		    }
		}
	    }
	}
